...
function App() {
  return (
    <div>
      ...
      <h2>Mon nom est : {formatName(user)}</h2>
      <h2>Mon nom est : {formatName1(user)}</h2>
      <h2>Mon nom est : {formatName2(user)}</h2>
      <h1>ok </h1>

      <h2 style = {myStyle}>Bonjour</h2>
      {disBonjour(user)}
      {numbersList}<br></br>
      {doubled}
      Le maximum du tableau est : {max} -- {max2}
      les valeurs paires sont : {pairCarre}
    </div>
  );
}
